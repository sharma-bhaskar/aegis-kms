...
/** SPI for LLM providers used by the AI policy and natural-language admin
  * features. Bundled backends: Anthropic, OpenAI, Ollama.
  *
  * The contract is deliberately narrow: Aegis-KMS only ever asks the LLM to
  * produce a structured plan of domain commands. The LLM never signs a
