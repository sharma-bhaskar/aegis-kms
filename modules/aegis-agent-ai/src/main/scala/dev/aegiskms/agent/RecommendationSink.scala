...
/** Sink for `AgentRecommendation` events.
  *
  * A streaming detector emits one record per detected deviation; whatever consumes the stream — the CLI tail,
  * the operator UI, the auto-responder (PR W3), webhooks (PR W3.b) — implements this trait.
  *
