...
/** Tests for `AegisHttpClient` using a hand-rolled `HttpPort` stub. We're not asserting on JDK HTTP
  * behaviour here — just on the wire shape Aegis emits and how the client interprets responses.
  */
final class AegisHttpClientSpec extends AnyFunSuite with Matchers:
  ...
  private final class RecordingPort(handler: HttpPort.Request => HttpPort.Response) extends HttpPort:
