...
/** A principal is any identity that can act against Aegis-KMS.
  *
  * Modeled as a sealed ADT so every call site is forced to pattern-match all
  * three kinds. In particular this means no code path can ever silently
  * conflate a short-lived `Agent` principal with a long-lived `Human`.
  */
...
object Principal:
  ...
  /** A long-lived service account, typically authenticated via mTLS or an
    * API key.
    */
  ...
  /** An AI agent acting on behalf of an operator.
    *
    * Agent credentials are always short-lived and carry an explicit allowlist
    * of operations. Every audit record written while acting as an agent
    * includes both the agent subject and the operator subject, so the chain
    * of responsibility is traceable.
    */
