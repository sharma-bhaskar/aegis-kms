...
final class ServerWiringSpec extends AnyFunSuite with Matchers:
  ...
  test("HttpRoutes assembles around an in-memory KeyService") {
    ...
    // 18 endpoints: 12 keys (create, get, activate, destroy, sign, verify, encrypt, decrypt,
    // wrap, unwrap, compromise, rotate) + agents/issue (#18) + agents/list (#101) +
    // agents/revoke (#102) + audit/query (#20) + advisor/scan (#28) + advisor/explain (#29).
    // Bump this when a new top-level REST surface lands.
    routes.serverEndpoints.size shouldBe 18
  }
